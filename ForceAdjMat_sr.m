...
matpath = '../s2_picked/maskR0.50_mat/*.mat';
imgdir = '../raw/';
outdir = '../s3_picked_withoutedge_sr/';
    ...
    for cycle = 1:nFrames %loop over these cycles 
        ...
        if ~exist([outputdir, 'stressRR_img/'], 'dir') mkdir([outputdir, 'stressRR_img/']); end
        if ~exist([outputdir, 'stressRR_array/'], 'dir') mkdir([outputdir, 'stressRR_array/']); end
        ...
        energyImgFilename = [[outputdir, 'stressRR_img/'], files(cycle).name(1:end-4),'.png'];  %output filename 
        energyArrayFilename = [[outputdir, 'stressRR_array/'], files(cycle).name(1:end-4),'.txt'];  %output filename 
...
%             err = 1000;
            ...
            if(length(particle(n).neighbours) > 0 && length(particle(n).alphas) > 0) % particle is in contact
                ...
                elastic_energy = sr_distribution(z,forces, alphas, betas, particle(n).fsigma, rm, px, verbose, E, nu, thickness);
...
%                 c_mask=((a.^2+b.^2)<=(rr)^2);      
                ...
                for m=1:length(forces) %for each contact
                    ...
                    if(forces(m) > fmin && err < emax && forces(m) < fmax) %is this a valid contact ?
                        ...
                        allContacts(aID).srdisp = elastic_energy;
                        allContacts(aID).totsr = totEnergy;
...
%         imwrite(bigEnergyImg,energyImgFilename);
        ...
        val_range = [0,0.0045];
        imagesc(bigEnergyImg, val_range); axis off; colormap(jet); colorbar;
    ...
    writetable(struct2table(rmfield(allContacts,'srdisp')), [[outputdir, 'csv/'], files(end).name(1:end-4), '.csv']);

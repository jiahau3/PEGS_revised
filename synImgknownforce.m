...
matpath='../s1/mat/*.mat';
outdir='../s2_1setOptimized/';
radius_ratio=0.35;
tF1=0.2;tF2=0.5;tF3=1.0;tF4=2.0; 
...
%         disp(['processing file ',fileName, ' containing ' ,num2str(N), ' particles']); %status indicator
    ...
    %             disp(['fitting force(s) to particle ',num2str(n)]); %status indicator
                if (particle(n).z > 0 )
                    ...
                    se = strel('disk', 2);
                    sigma = 2;
                    template = template.*(template > 0.2);                    
...
%                     template = imgaussfilt(template, sigma);
    %                 template = imadjust(template, [0 0.85]);
                    ...
                    for k=1:NumOfCombination
                        ...
                        for i=1:z
                            forces(i) = tS(k,i)*particle(n).f*particle(n).contactG2s(i)/cg2s;
                        end
                        
%                         for i=1:z
%                             forces(i)=str2num(matpath(11:end-4))*9.8e-3;
%                         end                        

...
function img = s2map(z, f, alpha, beta, fsigma, rm, px, verbose, E, nu, thickness)
    ...
    parfor (x=1:px) %loop throgh image width
        ...
        for y=1:px  %loop throgh image height
            if ((xx(x)^2+xx(y)^2)<=rm^2) %check if we are actually inside
                xRow(y) = s2(xx(x), xx(y), z, f, alpha, beta, fsigma, rm, E, nu, thickness);  %call the StressEngine to compute the photoelastic response at each pixel
            end
        end

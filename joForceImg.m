...
%         img1 = uint8(arrayfun(@(x) find(abs(d(:)-x)==min(abs(d(:)-x))),img));
        % f2 = figure('Visible', 'off');
        % img1 = uint8(255 * mat2gray(img));
        % ax2 = subplot(1,2,2);
        % image(img1); colormap(gray);
        % pbaspect(ax2, [1 0.9 1]);
        % axis off;
        % drawnow
%         display(num2str(f), num2str(alpha));
    end
end
